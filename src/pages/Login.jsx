...
export default function Login() {
  ...
  const handleResetPassword = async (e) => {
    ...
    try {
      ...
      setMessage('Te hemos enviado un correo con el enlace para restablecer tu contraseña. (Revisa tu carpeta de Spam)')
    } catch (err) {
      console.error(err)
    ...
    <div style={{ animation: 'fadeIn 0.3s ease' }}>
      {mode === 'login' ? (
        <>
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', margin: '0.2rem 0', flexWrap: 'wrap', gap: '0.5rem' }}>
              ...
              <span onClick={() => { setMode('forgot'); setError(''); setMessage(''); }} style={{ color: '#11683E', fontSize: '0.85rem', textDecoration: 'none', fontWeight: 'bold', cursor: 'pointer' }}>¿Olvidaste tu contraseña?</span>
            </div>
        ...
        <div style={{ animation: 'fadeIn 0.2s ease' }}>
          <h2 style={{ fontSize: '2rem', marginBottom: '0.5rem', color: '#1C2B23', fontFamily: 'Fraunces' }}>Recuperar Contraseña</h2>
          <p style={{ color: '#5F7368', marginBottom: '2rem' }}>Ingresa tu correo electrónico para enviarte un enlace de recuperación.</p>

          <form onSubmit={handleResetPassword} style={{ display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
            {error && <div style={{ color: 'red', fontSize: '0.9rem', textAlign: 'center', background: '#FEE2E2', padding: '0.5rem', borderRadius: '8px' }}>{error}</div>}
            {message && <div style={{ color: '#166534', fontSize: '0.9rem', textAlign: 'center', background: '#DCFCE7', padding: '0.5rem', borderRadius: '8px' }}>{message}</div>}
            ...
            <button type="submit" disabled={loading} style={{ display: 'flex', justifyContent: 'center', gap: '0.5rem', background: '#11683E', color: 'white', border: 'none', padding: '1rem', borderRadius: '50px', fontWeight: 'bold', fontSize: '1rem', cursor: loading ? 'not-allowed' : 'pointer', marginTop: '1rem', opacity: loading ? 0.7 : 1, transition: 'transform 0.2s', boxShadow: '0 4px 10px rgba(17, 104, 62, 0.2)' }}
              ...
              onMouseOut={e => e.currentTarget.style.transform = 'scale(1)'}
            >
              {loading ? <Loader2 className="spinner" size={20} /> : 'Enviar Enlace'}
            </button>
            <button type="button" onClick={() => { setMode('login'); setError(''); setMessage(''); }} style={{ background: 'none', border: 'none', color: '#64748B', fontWeight: 'bold', cursor: 'pointer', padding: '0.5rem' }}>
